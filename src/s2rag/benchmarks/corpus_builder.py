...
from s2rag.benchmarks.schemas import BenchmarkExample
...
def build_example_corpus(
    ...
    extraction_client,
) -> BuiltBenchmarkCorpus:
    if extraction_client is None:
        raise ValueError("LLM extraction client is required for benchmark corpus construction")
    return _build_llm_example_corpus(example, extraction_client)


def build_sentence_fixture_corpus(
    example: BenchmarkExample,
) -> BuiltBenchmarkCorpus:
    """Build deterministic sentence facts for unit tests only."""
    return _build_sentence_fact_corpus(example)


def build_native_passage_evaluation_corpus(
    example: BenchmarkExample,
) -> BuiltBenchmarkCorpus:
    """Build the evaluator-owned passage ID view for native external rankings."""
    return _build_sentence_fact_corpus(example)


def _build_sentence_fact_corpus(
    example: BenchmarkExample,
) -> BuiltBenchmarkCorpus:
    ...
    for passage in example.passages:
        ...
        documents.append(
            Document(
                ...
                or f"benchmark://{example.dataset}/{example.example_id}/{passage.passage_id}",
                domain="benchmark",
            )
        )
        ...
        for sentence_index, sentence in enumerate(passage.sentences):
            ...
            chunk_to_passage[chunk_id] = passage.passage_id
            chunks.append(
                Chunk(
                    chunk_id=chunk_id,
                    document_id=document_id,
            ...
            for concept_name in concept_names[:8]:
                ...
                concept_id = concept_ids_by_name.setdefault(
                    normalized, _id("ent", f"concept:{normalized}")
                )
                entities.setdefault(
                    concept_id,
                    Entity(
            ...
            if len({argument.entity_id for argument in arguments}) < 2:
                ...
                entities[sentence_entity_id] = Entity(
                    entity_id=sentence_entity_id,
                    canonical_name=f"sentence {sentence_index}",
                    entity_type="sentence_anchor",
    ...
    path = [
        title_entity_by_passage[item]
        for item in example.gold_path
        if item in title_entity_by_passage
    ]
    return BuiltBenchmarkCorpus(
        ...
        bridge_entity_ids=bridge_entity_ids,
        gold_path=path,
...
def _build_llm_example_corpus(
    example: BenchmarkExample,
    extraction_client,
) -> BuiltBenchmarkCorpus:
    template = _build_sentence_fact_corpus(example)
    chunks = template.corpus.chunks
    entities = canonicalize_entities(extract_entities_with_llm(chunks, extraction_client))
    facts = extract_facts_with_llm(chunks, entities, extraction_client)
    fact_to_passage: dict[str, str] = {}
    gold_fact_ids: set[str] = set()
    covered_gold_chunks: set[str] = set()
    for fact in facts:
        passages = {
            template.chunk_to_passage[chunk_id]
            for chunk_id in fact.evidence_chunk_ids
            if chunk_id in template.chunk_to_passage
        }
        if not passages:
            continue
        fact_to_passage[fact.hyperedge_id] = sorted(passages)[0]
        gold_chunks = set(fact.evidence_chunk_ids) & template.gold_chunk_ids
        if gold_chunks:
            gold_fact_ids.add(fact.hyperedge_id)
            covered_gold_chunks.update(gold_chunks)
    coverage = (
        len(covered_gold_chunks) / len(template.gold_chunk_ids) if template.gold_chunk_ids else 0.0
    )
    return BuiltBenchmarkCorpus(
        corpus=Corpus(
            documents=template.corpus.documents,
            chunks=chunks,
            entities=entities,
            evidence_hyperedges=facts,
            graph_version=f"{example.dataset}-{example.example_id}-llm-v1",
        ),
        gold_fact_ids=gold_fact_ids,
        gold_chunk_ids=template.gold_chunk_ids,
        bridge_entity_ids=template.bridge_entity_ids,
        gold_path=template.gold_path,
        fact_to_passage=fact_to_passage,
        chunk_to_passage=template.chunk_to_passage,
        gold_passage_ids=template.gold_passage_ids,
        extraction_coverage=coverage,
    )


def build_suite_corpus(
    examples: list[BenchmarkExample],
    extraction_client,
) -> BuiltBenchmarkSuiteCorpus:
    documents, chunks, entities, facts = {}, {}, {}, {}
    training_pairs = []
    bridge_by_question = {}
    for example in examples:
        built = build_example_corpus(example, extraction_client)
    ...
    return BuiltBenchmarkSuiteCorpus(
        corpus=Corpus(
            ...
            entities=list(entities.values()),
            evidence_hyperedges=list(facts.values()),
            graph_version="benchmark-suite-v1",
        ),
        training_pairs=training_pairs,
        bridge_by_question=bridge_by_question,
    )
